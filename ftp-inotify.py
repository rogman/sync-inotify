...
# 配置上传的FTP账号
#
UPLOAD_FTP_HOST = 'v0.ftp.upyun.com'
UPLOAD_FTP_USER = 'the-test/the-test'
UPLOAD_FTP_PASS = 'the-test'
...
# 又拍云的FTP主机地址
if UPLOAD_FTP_HOST:
    synclogger.debug('Found host for upyun: %s.' % (UPLOAD_FTP_HOST))
else:
    synclogger.error('The host for upyun MUST be set.')
    print >>sys.stderr, 'The host for upyun MUST be set.' % (UPLOAD_FTP_HOST)
    sys.exit()

# 又拍云的FTP用户名
if UPLOAD_FTP_USER:
    synclogger.debug('Found username for upyun: %s.' % (UPLOAD_FTP_USER))
else:
    synclogger.error('The username for upyun MUST be set.')
    print >>sys.stderr, 'The username for upyun MUST be set.' % (UPLOAD_FTP_USER)
    sys.exit()

# 又拍云的FTP密码
if UPLOAD_FTP_PASS:
    synclogger.debug('Found password for upyun: %s.' % (UPLOAD_FTP_PASS))
else:
    synclogger.error('The password for upyun MUST be set.')
    print >>sys.stderr, 'Found password for upyun: %s.' % (UPLOAD_FTP_PASS)
    sys.exit()
